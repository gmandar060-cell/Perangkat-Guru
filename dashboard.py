...
def render_logo(width=70):
    st.markdown(
        f"""
        <div style="display:flex;justify-content:center;align-items:center;margin-bottom:10px;">
            <img src="{LOGO_URL}" width="{width}" height="{width}" alt="Logo SIAP AJAR 22">
        </div>
        """,
...
def run():
    ...
    st.markdown(
        f"""
        <div class="app-header" style="background:linear-gradient(135deg, #0F172A 0%, #1E3A8A 55%, #2563EB 100%);border-radius:16px;padding:28px 32px;margin-bottom:24px;box-shadow:0 10px 25px -5px rgba(15, 23, 42, 0.12);">
            <h1 style="font-size:26px;font-weight:800;margin:0 0 6px 0;color:#FFFFFF;">Selamat Berkarya, {st.session_state.user_name}</h1>
            <p style="font-size:14px;color:#CBD5E1;margin:0;line-height:1.5;">Susun perangkat pembelajaran secara terstruktur dan ekspor langsung ke Microsoft Word melalui platform SIAP AJAR 22.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if st.session_state.hasil_teks:
        ...
        st.markdown(f"""<div class="paper-a4" style="background:#FFFFFF;border:1px solid #CBD5E1;box-shadow:0 12px 30px rgba(0,0,0,0.08);padding:44px 50px;margin:16px auto;border-radius:4px;max-width:900px;color:#0F172A;">{preview_html}</div>""", unsafe_allow_html=True)
